# CLAUDE.md — Internet But Fun

Standing context for Claude Code sessions in this repository. Read this before doing anything.
...
- Secrets: `DATABASE_URL` and `ACTOR_COOKIE_SECRET` live in `.env.local` only (gitignored). `.env.example` carries placeholders. Secrets never appear in client code, logs, error responses, or `NEXT_PUBLIC_*`.
...
## Next: Backend Milestone 03 — first write path (spec not yet written)

Scope when drafted: `join` with both branches — empty-queue shortcut (eligible actor clicks Make your move → active turn created immediately + token minted) and normal queue insertion; first real events (`queue_joined`, `turn_started`); the turn-token lifecycle decision; **atomicity**: claim-check + turn-creation in the empty-queue shortcut must be transactional (two simultaneous first visitors must not both go active). The `queue_entries` status lifecycle is schema-defined but contract-undefined — the M03 spec must define it. Timers per contract: 15s ready-check (client counts from receipt; server enforces at 20s from offer), 30s active turn. Expiry never consumes the actor's claim; expired/skipped actors leave the queue and may rejoin.

After M03: start-turn, submit-move transaction (validate actor/turn/epoch/version/move → append event → update epoch → create claim), StatusDelta + polling, reversal animation, live pending-move spectating (`activeTurn.pendingMove`), drag-to-turn, epoch completion lifecycle.
